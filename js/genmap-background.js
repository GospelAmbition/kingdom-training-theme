...
let isTyping = false;
let currentPlatform = 'desktop'; // Track current platform

let textPromptElement = null;
let particlesContainer = null;
let frameContainer = null;
let playerScreen = null;
let youtubeLayer = null;
let particles = [];
let frameCount = 0;
const maxFrames = 6;
...
// Typing animation
function typeText() {
    if (isTyping || !textPromptElement) return;
    isTyping = true;
...
class Particle {
    constructor(type = 'data', forceLeft = false) {
        if (!particlesContainer || !textPromptElement) return;
...
function generateVideoFrame() {
    if (!frameContainer || !playerScreen) return;
...
function initGenMapAnimation() {
    // Get elements
    textPromptElement = document.getElementById('genmap-text-prompt');
    particlesContainer = document.getElementById('genmap-particles-container');
    frameContainer = document.getElementById('genmap-frame-container');
    playerScreen = document.getElementById('genmap-player-screen');
    youtubeLayer = document.querySelector('.genmap-background');
    
    // Only initialize if all required elements exist
    if (!textPromptElement || !particlesContainer || !frameContainer || !playerScreen || !youtubeLayer) {
        return;
    }
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initGenMapAnimation);
} else {
    initGenMapAnimation();
}
